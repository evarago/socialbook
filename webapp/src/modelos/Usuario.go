...
func BuscarUsuarioCompleto(usuarioID uint64, r *http.Request) (Usuario, error) {
	...
	//Uso do select especifico para concorrencia
	for i := 0; i < 4; i++ {
		select {
...
func BuscarSeguindo(canal chan<- []Usuario, usuarioID uint64, r *http.Request) {
	...
	var seguindos []Usuario
	if erro = json.NewDecoder(response.Body).Decode(&seguindos); erro != nil {
		canal <- nil
		return
	}
...
// BuscaPublicacoes chama a API para buscar as publicacoes do usuário
